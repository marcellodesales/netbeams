...
/*
	New source code. There is a bug which return wrong pageX and pageY values when an
	object is in another object which has SCROLL bars
*/
p.getPageX = function() {var parent = document.getElementById(this.parent.id); return (this.isChild)? this.parent.getPageX()+(this.x||0) - ((parent != null)? parent.scrollLeft: 0) : this.x||0}; //! Overwritten by NS4
p.getPageY = function() {var parent = document.getElementById(this.parent.id); return (this.isChild)? this.parent.getPageY()+(this.y||0) - ((parent != null)? parent.scrollTop: 0) : this.y||0}; //! Overwritten by NS4
...
/*********************************************************************************************
	...
	I don't know whether this is a BUG or not but when the user set the class name, he want to
	set the class name for the CSS style for the layer, NOT changing the _className of a layer.
	The ORIGINAL CODE tried to changed the _className which destroyed the layer's class name
	and screwed up all the inheritance hirachy.
	
	ONE problem this caused is the DragEvent.enableDragEvent function. This function will check
	the layer's class name and make sure the layer is a type of DynLayer. If the layer's _className
	is changed, the DragEvent DOES NOT regonize the layer as a type of DynLayer anomore.
	
	
	For example: We defined a new layer class called 'MyLayer' which extends DynLayer
	
	var myLayer = new MyLayer();
	alert(myLayer._className);					// this will print "MyLayer"
	myLayer.setClass("mylayer_css_style")		// this will NOT only change the CSS style BUT ALSO changed the _className
	alert(myLayer._className);					// this will print "mylayer_css_style"
	
	Because of the BUG, myLayer CAN NOT be used as a layer anymore. It can not have drag and drop event 
	and maybe other feature that a layer has.

 *********************************************************************************************/

//	this._className=c;							// REMOVE THIS ORIGINAL CODE
