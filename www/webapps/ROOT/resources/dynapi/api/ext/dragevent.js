...
/*********************************************************************************************************/
/*
	There is a BUG that does not position the drag icon correcly when the window is scrolled down or left.
*/
			// if icon is fixed width then center at pointer
			try {
				if(ic.w!=o.w||ic.h!=o.h) ic.setLocation(document.body.scrollLeft + e.getPageX()-(ic.w/2), document.body.scrollTop + e.getPageY()-(ic.h/2));
			} catch (e) {
				// if browser does not support document.body.scrollLeft or scrollTop, try the old source code
				if(ic.w!=o.w||ic.h!=o.h) ic.setLocation(e.getPageX()-(ic.w/2),e.getPageY()-(ic.h/2));
			}
...
//		if (lyr.parent.DragDrop) lyr.parent.DragDrop(lyr,e.getPageX(),e.getPageY());		// ORIGINAL SOURCE


/*
	There is a BUG that cause the drag and drop event fail when the window is scrolled down or to the right.
	Below is the fixed code.
*/
/**********************************************************************************************************************************************/
		try {
			// try the new way
			if (lyr.parent.DragDrop) lyr.parent.DragDrop(lyr,e.getPageX() + document.body.scrollLeft,e.getPageY() + document.body.scrollTop);
		} catch (ex) {
			// if failed, try the original way
			if (lyr.parent.DragDrop) lyr.parent.DragDrop(lyr,e.getPageX(),e.getPageY());		// ORIGINAL SOURCE
		}
/**********************************************************************************************************************************************/
...
/*
	Set the drag icon. This function is MODIFIED from the ORIGINAL api. There was a little bug
	that causes problem when a NEW drag icon is set. The OLD icon still active so I added a few
	line to REMOVE the OLD icon before setting the NEW icon.
*/
DynLayer.prototype.setDragIcon = function(icon){
	if(!icon) return;
	if (this._dragIcon) this.removeDragIcon();			// remove the current drag icon
...
/*
	Get the current drag icon. Return NULL if it does not exist. This function is added to the API
*/
DynLayer.prototype.getDragIcon = function() {
	if(!this._dragIcon) return null;
...
/*
	Remove the drag icon. This function is added to the API.
*/
DynLayer.prototype.removeDragIcon = function(){
	if(!this._dragIcon) return;
	this._dragIcon._dragOrg = null;
	dynapi.document.removeChild(this._dragIcon);
	this._dragIcon = null;
};
